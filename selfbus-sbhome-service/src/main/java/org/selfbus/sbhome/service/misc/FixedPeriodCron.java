...
/**
 * This class can parse a subset of crontab expressions and return useful information such as, given
 * a reference date, the date before, the date after, the period and the next run time.
 * 
 * From: http://code.google.com/p/java-crontab-expression
 * 
...
public class FixedPeriodCron
{
   ...
   public static final int[] ranges = { MIN_MINUTES, MAX_MINUTES, MIN_HOURS, MAX_HOURS, MIN_DAYS, MAX_DAYS, MIN_MONTHS,
      MAX_MONTHS, MIN_DAYS_OF_WEEK, MAX_DAYS_OF_WEEK };
   ...
   /**
    * The parsed crontab expression fields ({minutes, hours, days, months, weekdays}).
    */
   @SuppressWarnings("unchecked")
   protected SortedSet<Integer>[] crontabSpec = new SortedSet[NUMBER_OF_FIELDS];

   /**
    * Creates a cron line object out of an expression.
    * 
    * <p>
    * REMEMBER: "&#42;&#47;2" in days means that day 1 is taken into consideration and then every 2
    * days, i.e. 1, 3, 5, ...<br/>
    * This is because the "2" there is a step modifier.
    * </p>
    * 
    * @param crontabExpression Crontab expression (see <i>crontab(5)</i> manpage).
    */
   public FixedPeriodCron(String crontabExpression) throws IllegalArgumentException
   {
      ...
      for (int field = 0; field < NUMBER_OF_FIELDS; ++field)
      {
         ...
         for (String crontabAtom : crontabAtoms)
         {
            ...
            {
               this.throwIllegalArgumentException("Wrong step (division) specification.");
            }
            else if (crontabAtomStep.length == 2)
            {
               ...
               {
                  this.throwIllegalArgumentException("Wrong step (divisor) specification.");
               }
            }
            /* ranges (hyphenated) */
            int rangeStart = 0;
            ...
            else
            {
               ...
               {
                  this.throwIllegalArgumentException("Wrong range specification.");
               }
               else if (crontabAtomRange.length == 2)
               {
                  try
                  {
                     rangeStart = Integer.valueOf(crontabAtomRange[0]);
                     rangeEnd = Integer.valueOf(crontabAtomRange[1]);
                  }
                  catch (Exception exception)
                  {
                     this.throwIllegalArgumentException("Wrong range specification.");
               ...
               else
               {
                  ...
                  {
                     this.throwIllegalArgumentException("Wrong number specification.");
                  }
               }
            }
   ...
   /**
    * Convenience function that builds a common format exception to be thrown when the arguments
    * passed are illegal.
    * 
    * @param cause The exception to be thrown.
    */
   protected void throwIllegalArgumentException(String cause) throws IllegalArgumentException
   {
      throw new IllegalArgumentException("Malformed crontab expression. " + cause
         + " Read crontab(5) manpage for further reference.");
   }
   ...
   /**
    * Returns the closest calendar previous (or equal) to the reference, in relation to the cron
    * expression.
    * 
    * <p>
    ...
    * @return The closest calendar previous (or equal) to the reference, in relation to the cron
    *         expression.
    */
   public Calendar getClosestDateBeforeOrSame(Calendar reference)
   {
      ...
      Integer[][][] specsForReferenceParent = new Integer[NUMBER_OF_FIELDS - 2][][];
      Integer[][] specsInOrder = new Integer[NUMBER_OF_FIELDS - 1][];
      for (int field = 0; field < NUMBER_OF_FIELDS - 2; ++field)
      {
         specsForReferenceParent[field] = new Integer[2][];
         /* first pass */
         List<Integer> specs = new ArrayList<Integer>(this.crontabSpec[field].headSet(fields[field] + 1));
         Collections.reverse(specs);
         specsForReferenceParent[field][0] = specs.toArray(typeParameter);
         /* second pass */
         specs = new ArrayList<Integer>(this.crontabSpec[field].tailSet(fields[field]));
         Collections.reverse(specs);
      ...
      List<Integer> specs = new ArrayList<Integer>(this.crontabSpec[MONTHS].headSet(fields[MONTHS] + 1));
      Collections.reverse(specs);
      List<Integer> specsTail = new ArrayList<Integer>(this.crontabSpec[MONTHS].tailSet(fields[MONTHS]));
      Collections.reverse(specsTail);
      specs.addAll(specsTail);
      specsInOrder[MONTHS] = specs.toArray(typeParameter);

      return this.findDate(-1, fields, reference.get(Calendar.YEAR), specsInOrder, specsForReferenceParent);
   }

   /**
    * Returns the closest calendar after the reference, in relation to the cron expression.
    * 
    * <p>
    * TODO: Re-think this method to make it shorter and faster.
    * </p>
    * 
    * @param reference Reference calendar (usually, "now").
    * @return The closest calendar after the reference, in relation to the cron expression.
    */
   public Calendar getClosestDateAfter(Calendar reference)
   {
      ...
      for (int field = 0; field < NUMBER_OF_FIELDS - 2; ++field)
      {
         specsForReferenceParent[field] = new Integer[2][];
         /* first pass */
         List<Integer> specs = new ArrayList<Integer>(this.crontabSpec[field].tailSet(fields[field]));
         specsForReferenceParent[field][0] = specs.toArray(typeParameter);
         /* second pass */
         specs = new ArrayList<Integer>(this.crontabSpec[field].headSet(fields[field] + 1));
         specsForReferenceParent[field][1] = specs.toArray(typeParameter);
         /* general order of specs */
         specs = new ArrayList<Integer>(this.crontabSpec[field]);
         specsInOrder[field] = specs.toArray(typeParameter);
      }
      List<Integer> specs = new ArrayList<Integer>(this.crontabSpec[MONTHS].tailSet(fields[MONTHS]));
      List<Integer> specsTail = new ArrayList<Integer>(this.crontabSpec[MONTHS].headSet(fields[MONTHS] + 1));
      specs.addAll(specsTail);
      specsInOrder[MONTHS] = specs.toArray(typeParameter);
   ...
   /**
    * Returns the number of milliseconds to the next match/run, relative to a calendar.
    * 
    * @param reference Reference calendar (usually, "now").
   ...
   /**
    * Returns the period span in milliseconds given a reference date. Takes the period where the
    * date of the calendar is in.
    * 
    * @param reference Reference calendar (usually, "now").
   ...
   /**
    * Searches for a cron matching date, forwards or backwards in time, from a given reference.
    * 
    * <p>
    * TODO: Re-think this method to make it shorter and faster.
    * </p>
    ...
    * @param specsInOrder Values coming from the crontab evaluation, sorted to be matched in order.
    * @param year The year of the reference date (not in the fields array to avoid confusion).
    * @param specsForReferenceParent Values coming from the crontab evaluation, sorted to be matched
    *           in order, given that the parent value matches the reference date field. Each
    *           containing two lists for the first and second passes (i.e. same year and the year
    *           before/after--depending on the direction).
    * @return The date looked for.
    */
   protected Calendar findDate(int direction, Integer[] fields, int year, Integer[][] specsInOrder,
      ...
      for (Integer aMonth : specsInOrder[MONTHS])
      {
         ...
         for (Integer aDay : specsForCurrentMonth)
         {
            ...
            if (!aMonth.equals(calendarHelper.get(Calendar.MONTH) + 1)
               || !this.crontabSpec[DAYS_OF_WEEK].contains(calendarHelper.get(Calendar.DAY_OF_WEEK) - 1))
            {
               continue;
            }
            ...
            {
               specsForCurrentDay = specsForReferenceParent[HOURS][pass];
            }
            else
            {
            ...
            if (specsForCurrentDay.length > 0)
            {
               ...
               {
                  specsForCurrentHour = specsForReferenceParent[MINUTES][pass];
               }
               else
               {
